...
项目提供了 `build_ipk.sh` 脚本，可以快速生成 IPK 安装包。

**注意**：脚本默认为 `aarch64_cortex-a53` 架构编译，如果你的设备是其他架构，请先修改脚本中的 `ARCH` 变量。

- **aarch64_cortex-a53**（默认编译架构）
- **aarch64_cortex-a72**
- **arm_cortex-a7**
- **arm_cortex-a9** 
- **mipsel_24kc**
...
编辑 `build_ipk.sh` 文件，修改 `ARCH` 变量：
```text
ARCH="你的设备架构"  # 例如：aarch64_cortex-a53
```

```bash
# 1. 克隆项目
git clone https://github.com/xiaciyidingde/luci-app-camtools.git
cd luci-app-camtools

# 2. 修改架构（如果需要）
# 编辑 build_ipk.sh，找到 ARCH 变量并修改为你的设备架构
# 常见架构：aarch64_cortex-a53, arm_cortex-a7, mipsel_24kc, x86_64
